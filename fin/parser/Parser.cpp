...
void Parser::ifStatementPrc()
{
    ...
    // Разбираем логическое выражение
    Item::ItemTypes expressionType = expressionPrc();
    checkBoolType(expressionType);

    // Генерируем условный переход если false (0)
    int falseBranchAddress = generateCode.getCmdCounter();
    generateCode.gen(0); // Временный адрес для перехода
    generateCode.gen(OVM::IFZ); // Переход если false (0)

    checkLex(Scanner::Lex::THEN);
    sequenceStatementsPrc();

    int elseBranchAddress = generateCode.getCmdCounter();
    generateCode.gen(0); // Временный адрес для перехода ELSE
    generateCode.gen(OVM::GOTO);

    // Заполняем адрес для false ветки
    int elseStart = generateCode.getCmdCounter();
    generateCode.getMemory()[falseBranchAddress] = elseStart;

    while (scanner.lex == Scanner::Lex::ELSIF)
    {
        checkLex(Scanner::Lex::ELSIF);

        // Разбираем ELSIF выражение
        expressionType = expressionPrc();
        checkBoolType(expressionType);

        falseBranchAddress = generateCode.getCmdCounter();
        generateCode.gen(0); // Временный адрес для перехода
        generateCode.gen(OVM::IFZ); // Переход если false (0)
        ...
        elseBranchAddress = generateCode.getCmdCounter();
        generateCode.gen(0); // Временный адрес для перехода ELSE
        generateCode.gen(OVM::GOTO);

        // Заполняем адрес для false ветки
        generateCode.getMemory()[falseBranchAddress] = generateCode.getCmdCounter();
    }
    ...
    // Заполняем адрес для ELSE ветки
    int endIf = generateCode.getCmdCounter();
    generateCode.getMemory()[elseBranchAddress] = endIf;
