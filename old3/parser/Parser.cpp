...
{
    // Начинаем с побитовых операций
    Item::ItemTypes left = bitExpressionPrc();

    // Проверяем, есть ли логическое сравнение
        ...
        scanner.lex == Scanner::Lex::GE)
    {
        ...
        // Удостоверимся, что сравниваются INTEGER'ы
        checkIntType(left);

        scanner.nextLex();

        // Получаем правую часть
        Item::ItemTypes right = bitExpressionPrc();
        checkIntType(right);

        // Генерируем корректное логическое сравнение
        generateCode.genComparison(scanner.getStringNameOfLex(operation));

        // Возвращаем INTEGER, потому что сравнение даст 0 или 1
...
Item::ItemTypes Parser::bitExpressionPrc()
{
    Item::ItemTypes left = bitTermPrc();

    while (scanner.lex == Scanner::Lex::OR) {
        scanner.nextLex();
        checkIntType(left);
        Item::ItemTypes right = bitTermPrc();
        checkIntType(right);
        generateCode.genOR();
    }
...
Item::ItemTypes Parser::bitTermPrc()
{
    Item::ItemTypes left = simpleExpressionPrc();

    while (scanner.lex == Scanner::Lex::AND) {
        scanner.nextLex();
        checkIntType(left);
        Item::ItemTypes right = simpleExpressionPrc();
        checkIntType(right);
        generateCode.genAND();
    }
...
Item::ItemTypes Parser::simpleExpressionPrc()
{
        ...
        scanner.lex == Scanner::Lex::MINUS)
    {
        auto operation = scanner.lex;
        scanner.nextLex();
        Item::ItemTypes nextType = termPrc();
        checkIntType(nextType);
...
Item::ItemTypes Parser::multiplierPrc()
{
    ...
    if (scanner.lex == Scanner::Lex::NOT) {
        scanner.nextLex();
        Item::ItemTypes t = multiplierPrc();
        checkIntType(t);
        generateCode.genNOT();
        return t;
    }
    ...
    return Item::ItemTypes::Integer; // заглушка
}
...
{
    // Разрешаем как Boolean, так и Integer — считаем, что они логически равны
    if (type != Item::ItemTypes::Boolean && type != Item::ItemTypes::Integer)
    {
        errorPtr->contextError("Ожидается логическое выражение (0 или 1)");
    }
}
